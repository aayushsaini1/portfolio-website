...
export function getExperimentsData() {
  const filePath = path.join(contentDirectory, 'experiments.json');
  const fileContents = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(fileContents);
}
...
export function getWorkDetailHtml(slug) {
  try {
    const filePath = path.join(contentDirectory, 'work', `${slug}.md`);
    ...
    console.error(`Error loading work details for project ${slug}:`, e);
  }
  return null;
}
